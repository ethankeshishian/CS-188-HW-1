...
# Name: Ethan
# Email: ethankeshishian@ucla.edu
...
class Lexicon:
    ...
    def __init__(self, file) -> None:
        ...
        for line in f:
            vals = line.split(" ")
            self.word_emb_dict[vals[0]] = list(map(float, vals[1:]))
        f.close()
        ##################################################
...
def main():
    ...
    log.info("Word vector of {} from {}: {}".format(
        args.word, args.embeddings, lexicon.get_vector(args.word)))

    log.info("{}\nCoding task 2: cosine similarity".format("-"*10))
    cossim_result = cossim(lexicon.get_vector(
        args.word), lexicon.get_vector("animal"))
    log.info("Cosine similarity between {} and {} is: {}".format(
        args.word, "animal", cossim_result))
    ...
    sim = lexicon.find_nearest_words(
        args.word, {args.word}, n=args.n, minus=args.minus, plus=args.plus)
    log.info("Top {} similar words of {} - {} + {} are: {}".format(args.n,
             args.word, args.minus, args.plus, sim))
